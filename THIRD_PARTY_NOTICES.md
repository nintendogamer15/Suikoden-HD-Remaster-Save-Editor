...
The standalone application also distributes the shared save-editor GUI framework `SaveEditor.Ui`, consumed as a git submodule under `external/save-editor-gui-framework` and pinned at commit 6ee70c4. nintendogamer15 licenses it under the Zero-Clause BSD license (`0BSD`); see `LICENSES/SaveEditor.Ui-0BSD.txt`. It in turn distributes `CommunityToolkit.Mvvm`, which the .NET Foundation and contributors license under MIT; see `LICENSES/CommunityToolkit.Mvvm-MIT.txt`.
