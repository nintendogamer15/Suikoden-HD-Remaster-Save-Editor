    ...
    public static void Build(Suikoden2Adapter adapter, SectionBuilder builder)
    {
        ...
        })
        {
            ...
            for (int index = 0; index < array.Count; index++)
            {
                ...
                builder.AddChoice(
                    ...
                    () =>
                    {
                        JsonObject currentItem = array[captured]!.AsObject();
                        return SectionText.FormatS2InventoryItem(inventory, currentItem["item_no"]!.GetValue<int>(), currentItem["use_cnt"]!.GetValue<int>());
                    },
                ...
                if (currentItem is { Category: Suikoden2ItemCategory.Regular, UseCount: > 1 })
                {
                    ...
                    builder.AddChoice(
                        ...
                        $"{path}[{index}].use_cnt",
                        () => array[captured]!.AsObject()["use_cnt"]!.GetValue<int>().ToString(CultureInfo.InvariantCulture),
        ...
        for (int index = 0; index < keyItems.Count; index++)
        {
            ...
            builder.AddChoice(
                ...
                () =>
                {
                    int current = keyItems[captured]!.GetValue<int>();
                    ...
                    return SectionText.S2ItemDisplayName(currentItem);
                },
                namedChoices.Keys.Order(StringComparer.OrdinalIgnoreCase),
